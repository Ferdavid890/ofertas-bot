...
def obtener_token_ebay():
    ...
    if response.status_code == 200:
        ...
        logging.info("[Token Manager] Token de eBay renovado.")
        return EBAY_TOKEN_CACHE["access_token"]
    else:
    ...
    for intento in range(intentos):
        try:
            ...
            if response.status_code == 200:
                ...
                logging.warning(f"[Rate Limiter] 429. Reintentando en {tiempo_espera:.2f}s...")
...
def buscar_ebay_recursivo_adaptativo(p_min, p_max, headers, stats, buying_option="FIXED_PRICE"):
    """Búsqueda adaptativa por rangos para Buy It Now (Listings)."""
    items_acumulados = []
    limit = 100
...
def buscar_auctions_hoy(headers, stats):
    """Obtiene subastas que terminan hoy (UTC) para posterior filtrado preciso en hora CDMX."""
    items_acumulados = []
    limit = 100
    hoy_utc_str = datetime.now(timezone.utc).strftime("%Y-%m-%d")
    
    # Filtramos por las subastas que expiran en la ventana del día actual en UTC
    url_base = f"https://api.ebay.com/buy/browse/v1/item_summary/search?q=Lorcana+PSA+10&filter=buyingOptions:AUCTION,endTime:[{hoy_utc_str}T00:00:00Z..{hoy_utc_str}T23:59:59Z]&limit={limit}&offset="
    ...
    for page in range(1, min(paginas_totales, 20)):
        offset = page * limit
        stats["consultas_ebay"] += 1
...
def barrido_listings_incremental_worker():
    """Ejecuta el escaneo de Listings (Buy It Now) llenando todas sus 15 columnas."""
    if not EXECUTION_LOCK.acquire(blocking=False):
        logging.warning("[Lock Global] Barrido de listings omitido por concurrencia.")
        return
    ...
    try:
        logging.info("--- [INICIO] Barrido incremental de Buy It Now (Listings) ---")
        sheet = obtener_cliente_sheets()
        ...
        logging.info(f"--- [FIN Listings] Completado en {tiempo_total:.2f}s | Stats: {stats} ---")
...
def proceso_fondo_matutino_worker():
    """Descarga subastas de hoy (filtrando hora CDMX) y ejecuta el barrido de Listings."""
    try:
        logging.info("--- [INICIO] Proceso Matutino: Subastas de hoy y Listings ---")
        sheet = obtener_cliente_sheets()
        token = obtener_token_ebay()
        headers = {
            "Authorization": f"Bearer {token}",
        ...
        logging.info("[Proceso Matutino] Consultando subastas en API de eBay...")
        ...
        for item in items_auctions:
            ...
            try:
                # Convertir timestamp ISO UTC a objeto Datetime en zona Horaria CDMX
                dt_utc = datetime.fromisoformat(item_end_time.replace("Z", "+00:00"))
                dt_cdmx = dt_utc.astimezone(tz_cdmx)
                
                # FILTRO CLAVE: Asegurar que la subasta realmente finaliza HOY en Hora CDMX
                if dt_cdmx.strftime("%Y-%m-%d") == hoy_cdmx_str:
                    ...
                    if item_id:
                        ...
                        if not existe:
                            ...
                            cierre_str = dt_cdmx.strftime("%Y-%m-%d %H:%M:%S") # Texto legible en col M
                            vendedor, location = extraer_info_vendedor_ubicacion(item)
                            ...
                            auctions_lote.append([
                                ...
                                0,                    # Col K
                                0,                    # Col L
                                cierre_str,           # Col M (scheduled_closing_time)
                                "Activa",             # Col N
                                item_url              # Col O
                            ])
            except Exception as parse_e:
                logging.error(f"Error procesando fecha de item {item.get('itemId')}: {parse_e}")
                continue

        if auctions_lote:
            ws_auctions.append_rows(auctions_lote, value_input_option='USER_ENTERED')
            logging.info(f"[Google Sheets] Insertadas {len(auctions_lote)} subastas que cierran hoy ({hoy_cdmx_str} CDMX).")
        else:
            logging.info("[Google Sheets] No se encontraron subastas nuevas que cierren hoy.")

        # Ejecutamos inmediatamente la captura completa de Listings
...
def revisar_y_actualizar_subastas_worker():
    """Sondeo de precios finales en subastas próximas a vencer."""
    try:
        sheet = obtener_cliente_sheets()
        ws_auctions = sheet.worksheet("Auctions")
...
def ejecutar_freeze_diario():
    ...
    hilo.start()
    return jsonify({"status": "success", "message": "Proceso matutino de subastas y listings iniciado."})
