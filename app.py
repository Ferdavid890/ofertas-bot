...
class EbaySearchEngine:
    QUERIES = ["Lorcana PSA 10"]  # igual que la version simple que si funcionaba
    RANGOS = [("0", "999999")]  # una sola pasada; la recursion divide sola si hay overflow
    CATEGORY_PARAM = ""  # sin categoria: "Lorcana PSA 10" como texto ya es suficientemente especifico
    LIMIT = 100
    MAX_ITEMS_POR_QUERY = 3000  # tope de seguridad para no colgar el proceso en plan free

    def buscar_recursivo(self, query, p_min, p_max, sort_order, buying_option, stats, extra_filter="", max_items=None):
        items_acumulados = {}
    ...
    def ejecutar_motor_general(self, buying_option, sort_order, extra_filter="", max_items=None):
        stats = {"total_reportado": 0}
        todos_items = {}
        for q in self.QUERIES:
            for r in self.RANGOS:
                resultados = self.buscar_recursivo(q, r[0], r[1], sort_order, buying_option, stats, extra_filter, max_items)
...
def sync_auctions():
    ...
    try:
        ...
        items, stats = search_engine.ejecutar_motor_general("AUCTION", "endingSoonest", max_items=2000)
        ...
        for idx, (item_id, it) in enumerate(items.items()):
            ...
            # Item nuevo: solo usamos itemEndDate si YA viene en el resumen de busqueda.
            # Si no viene, lo descartamos (sin pagar una llamada de detalle extra por
            # cada uno) - esto es lo que hacia la version simple que si funcionaba.
...
def iniciar_scheduler():
    ...
    scheduler.add_job(sync_auctions, IntervalTrigger(minutes=15), id="sync_auctions")
    scheduler.start()
    recuperar_snipers_pendientes()
    log.info("Scheduler iniciado: listings cada 1h, auctions detectadas cada 15min, snipers puntuales T-60s/T-2s.")
