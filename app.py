...
class EbaySearchEngine:
    QUERIES = ['Lorcana "PSA 10"', 'Lorcana PSA10', 'Lorcana "Gem Mint 10"']  # cubre "PSA 10" con espacio, "PSA10" pegado, y "Gem Mint 10"
    RANGOS = [("0", "999999")]  # una sola pasada; la recursion divide sola si hay overflow
    CATEGORY_PARAM = "&category_ids=183454"  # Disney Collectible Individual Card Games - confirmado correcto
    LIMIT = 100
    MAX_ITEMS_POR_QUERY = 3000  # tope de seguridad para no colgar el proceso en plan free

    def buscar_recursivo(self, query, p_min, p_max, sort_order, buying_option, stats, extra_filter="", detener_en_fecha=None):
        items_acumulados = {}
        url = self._build_url(query, p_min, p_max, sort_order, buying_option, 0, extra_filter)
        ...
        if "warnings" in data:
            log.warning(f"[EbaySearch] eBay devolvio warnings (posible filtro ignorado): {data['warnings']}")

        # NOTA: se elimino la particion recursiva por rango de precio (los precios se
        # concentran en un rango angosto y dividir no ayudaba). En su lugar paginamos
        # directo hasta un tope fijo. Ademas, cuando se especifica detener_en_fecha
        # (usado por Auctions con sort=endingSoonest), cortamos la paginacion en cuanto
        # aparece el primer item que cierra despues de esa fecha limite: como el orden
        # es "lo que cierra mas pronto primero", todo lo que sigue en las paginas
        # restantes cierra aun mas tarde, asi que seguir pidiendo esas paginas seria
        # gastar cuota en items que de todos modos vamos a descartar.
        ...
        for p in range(paginas):
            ...
            if offset == 0:
                ...
                paged_url = self._build_url(query, p_min, p_max, sort_order, buying_option, offset, extra_filter)
            ...
            for it in data_p.get("itemSummaries", []):
                if detener_en_fecha:
                    end_str = it.get("itemEndDate", "")
                    if end_str:
                        try:
                            dt_end = datetime.fromisoformat(end_str.replace("Z", "+00:00"))
                            if dt_end > detener_en_fecha:
                                detener = True
                                continue  # este item cierra despues de hoy: no lo guardamos
                        except ValueError:
                            pass
            ...
            if detener:
                log.info(f"[EbaySearch] '{query}' {buying_option}: corte anticipado en pagina {p+1}/{paginas} "
                         f"(ya aparecieron items que cierran despues de hoy)")
                break
        return items_acumulados

    def _build_url(self, query, p_min, p_max, sort_order, buying_option, offset, extra_filter=""):
        return (
    ...
    def ejecutar_motor_general(self, buying_option, sort_order, extra_filter="", detener_en_fecha=None):
        stats = {"total_reportado": 0}
        todos_items = {}
        for q in self.QUERIES:
            for r in self.RANGOS:
                resultados = self.buscar_recursivo(q, r[0], r[1], sort_order, buying_option, stats, extra_filter, detener_en_fecha)
...
def sync_listings():
    ...
    try:
        ...
        for item_id, it in items.items():
            ...
            title = it.get("title", "")
            price_nuevo = float(it.get("price", {}).get("value", 0))
...
def sync_auctions():
    ...
    try:
        # El filtro server-side itemEndDate de eBay requiere autenticacion de usuario
        # (no funciona con nuestro token de aplicacion client_credentials, y ademas
        # rompia el filtro buyingOptions al combinarse). En vez de eso, aprovechamos
        # que la busqueda va ordenada por endingSoonest: en cuanto aparece un item que
        # cierra despues de hoy, sabemos que todo lo que sigue tambien cierra despues,
        # y cortamos la paginacion ahi (ver detener_en_fecha en buscar_recursivo).
        hoy_cdmx_fin = datetime.now(TZ_CDMX).replace(hour=23, minute=59, second=59, microsecond=0)
        limite_utc = hoy_cdmx_fin.astimezone(timezone.utc)

        items, stats = search_engine.ejecutar_motor_general("AUCTION", "endingSoonest", detener_en_fecha=limite_utc)
        ...
        for idx, (item_id, it) in enumerate(items.items()):
            ...
            # Item nuevo: primero intentamos leer itemEndDate del PROPIO resultado
            # de busqueda (Browse API lo incluye en el summary para items de subasta).
            # Solo si no viene ahi, pedimos el detalle completo (1 llamada extra).
...
def iniciar_scheduler():
    ...
    # No auto-arrancan al iniciar: mañana los disparas tu manualmente por HTTP
    # (/ejecutar-listings primero, confirmas que salio bien, LUEGO /ejecutar-subastas)
    # para no gastar cuota en los dos a la vez si algo sale mal en el primero.
    scheduler.add_job(sync_listings, IntervalTrigger(hours=1), id="sync_listings")
    scheduler.add_job(sync_auctions, IntervalTrigger(minutes=30), id="sync_auctions")
    scheduler.start()
    recuperar_snipers_pendientes()
    log.info("Scheduler iniciado: listings cada 1h, auctions detectadas cada 30min, snipers puntuales T-60s/T-2s.")
