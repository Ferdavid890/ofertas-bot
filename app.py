...
def verificar_venta_real(url):
    """
    Consulta la pagina publica del item en ebay.com (no es la Browse API - esa ya
    no responde nada util una vez que el item termino) para distinguir una venta
    real ("Sold") de una publicacion dada de baja sin vender ("Ended"). Devuelve
    "Sold", "Ended", "Desconocido" (pagina cargo pero no matcheo ningun patron
    conocido - eBay pudo haber cambiado el diseño) o None (fallo la peticion,
    hay que reintentar despues).
    """
    try:
        ...
        if resp.status_code != 200:
            log.info(f"[VerificarVenta] DIAGNOSTICO status={resp.status_code} para {url} "
                     f"| primeros 200 chars: {resp.text[:200]!r}")
            return None
        texto = resp.text
        if re.search(r"This listing sold on", texto, re.IGNORECASE):
            return "Sold"
        if re.search(r"ended by the seller|This listing has ended|no longer available", texto, re.IGNORECASE):
            return "Ended"
        return "Desconocido"
    except Exception as e:
        log.warning(f"[VerificarVenta] Error consultando {url}: {e}")
        return None
...
def verificar_ventas_pendientes():
    ...
    aun no tienen venta_confirmada y valida contra la pagina publica de eBay si
    fue una venta real o solo se dio de baja. Procesa un LOTE acotado por corrida
    (no todo de golpe) para no tardarse una eternidad ni mandar una rafaga de
    peticiones sospechosa a eBay - en el rollover de medianoche pueden pasar a
    Vendido cientos/miles de items a la vez, y esto se va poniendo al dia en
    varias corridas sucesivas de este job (cada 20 min).
    """
    LOTE = 40
    ...
    try:
        ...
        for row_num, r in pendientes:
            ...
            if not url:
                continue
            resultado = verificar_venta_real(url)
            if resultado:
                actualizaciones.append({
                    "range": gspread.utils.rowcol_to_a1(row_num, COL_VENTA_CONFIRMADA),
                    "values": [[resultado]]
                })
            time.sleep(random.uniform(1.0, 2.0))  # pacing - pagina publica, no API oficial
...
def iniciar_scheduler():
    ...
    # verificar_ventas_pendientes DESACTIVADO: eBay bloquea con 403 (bot-block, "Error
    # Page") toda peticion que llega desde la IP del datacenter de Render, sin importar
    # los headers usados - confirmado en pruebas reales el 16-ago-2026. No es arreglable
    # sin pagar proxies residenciales. Ver conversacion con el usuario para la
    # alternativa (Trading API GetItem con User token, o usar 'bids>0' como ground
    # truth gratis para Auctions).
    ...
    log.info("Scheduler iniciado: listings cada 1h, auctions detectadas 1 vez/dia (00:05 CDMX), "
             "snipers puntuales T-60s/T-2s/T-1s. Verificacion de ventas (scraping) desactivada - eBay bloquea con 403.")
...
@app.route("/setup-user-token", methods=["GET"])
def setup_user_token():
    """
    TEMPORAL - usar UNA vez para intercambiar el authorization code (de la pantalla
    de consentimiento OAuth) por el refresh_token de usuario, y despues BORRAR esta
    ruta. No expone EBAY_CLIENT_SECRET en ningun lado - lo usa server-side, leido
    de la variable de entorno.
    Uso: /setup-user-token?code=EL_CODE_DE_LA_URL&runame=TU_RUNAME
    """
    code = request.args.get("code")
    runame = request.args.get("runame")
    if not code or not runame:
        return jsonify({"error": "faltan parametros ?code= y ?runame="}), 400
    client_id = os.environ.get("EBAY_CLIENT_ID")
    client_secret = os.environ.get("EBAY_CLIENT_SECRET")
