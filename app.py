...
class EbaySearchEngine:
    ...
    RANGOS = [("0", "999999")]  # una sola pasada; la recursion divide sola si hay overflow (>9000)
    ...
    def buscar_recursivo(self, query, p_min, p_max, sort_order, buying_option, stats):
        ...
        if total_reportado > 9000 and (float(p_max) - float(p_min)) > 1:
        ...
        paginas = ceil(min(total_reportado, 10000) / self.LIMIT) if total_reportado > 0 else 1
...
def sync_listings():
    ...
    try:
        ...
        log.info(f"[Listings] Nuevos agregados: {len(filas_nuevas)} | Duracion: {round(time.time()-inicio,1)}s")
...
def sync_auctions():
    ...
    try:
        ...
        log.info(f"[Auctions] Nuevas: {len(filas_nuevas)} | Snipers activos: {len(scheduled_ids)} | Duracion: {duracion}s")
    except Exception as e:
