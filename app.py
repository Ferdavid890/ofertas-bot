...
class EbaySearchEngine:
    ...
    DEBUG_KEYWORD = "razoul"  # TEMPORAL - quitar tras resolver Bug 9
    ...
    def buscar_recursivo(self, query, p_min, p_max, sort_order, buying_option, stats, category_param="", extra_filter="", max_items=None):
        ...
        for p in range(paginas):
            ...
            for it in pagina_items:
                titulo = it.get("title", "")
                if self.DEBUG_KEYWORD in titulo.lower():
                    debug_matches.append({
                        "itemId": it.get("itemId"),
                        "title": titulo,
                        "buyingOptions": it.get("buyingOptions"),
                        "itemEndDate": it.get("itemEndDate"),
                        "campos_faltantes_critico": [c for c in ["itemId", "price", "itemWebUrl"] if c not in it],
                        "tiene_currentBidPrice": "currentBidPrice" in it,
                        "paso_filtro": MarketAnalyzer.validar_item_critico(it) and MarketAnalyzer.es_carta_psa_10(titulo)
                    })
                if MarketAnalyzer.validar_item_critico(it) and MarketAnalyzer.es_carta_psa_10(titulo):
                    items_acumulados[it["itemId"]] = it
            time.sleep(random.uniform(0.15, 0.35))
        if debug_matches:
            log.info(f"[DEBUG] '{query}' {buying_option}: coincidencias con '{self.DEBUG_KEYWORD}' en crudos ({len(debug_matches)}): {debug_matches}")
        else:
            log.info(f"[DEBUG] '{query}' {buying_option}: SIN coincidencias con '{self.DEBUG_KEYWORD}' en {raw_total_crudo} crudos")
...
def iniciar_scheduler():
    ...
    log.info("Scheduler iniciado: listings cada 1h, auctions detectadas 1 vez/dia (00:05 CDMX), snipers puntuales T-60s/T-2s.")
