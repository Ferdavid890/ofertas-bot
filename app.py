...
class EbayAuthManager:
    LIMITE_DIARIO_SEGURO = 4500  # margen bajo el limite real de 5000/dia de eBay
    ...
    def __init__(self):
        ...
        self.call_count = {"fecha": None, "total": 0}
    ...
    def peticion_con_retry(self, url, max_intentos=3, timeout=(5, 15)):
        usadas_hoy = self._registrar_llamada()
        if usadas_hoy > self.LIMITE_DIARIO_SEGURO:
            log.error(f"Limite diario seguro de eBay alcanzado ({usadas_hoy}/{self.LIMITE_DIARIO_SEGURO}). "
                      f"Saltando esta llamada para no tronar la cuota. Se libera manana.")
            return None
        ultimo_status = None
        ultimo_texto = ""
        for intento in range(max_intentos):
            try:
                ...
                if response.status_code == 200:
                    ...
                    if "request limit" in cuerpo.lower() or "daily" in cuerpo.lower():
                        # Limite diario de la app agotado: reintentar no sirve de nada hoy.
                        log.error(f"eBay 429 - LIMITE DIARIO AGOTADO para {url}: {cuerpo}")
                        return None
                    espera = (2 ** (intento + 1)) + random.uniform(1, 2)
                    log.warning(f"eBay 429 (rate limit) para {url}, reintentando en {espera:.1f}s")
                    time.sleep(espera)
...
class EbaySearchEngine:
    QUERIES = ["Lorcana PSA 10", "Lorcana Gem Mint 10"]  # cubren las 2 formas de nombrar el grado; se quitaron variantes redundantes para ahorrar cuota
    RANGOS = [("0", "999999")]  # una sola pasada; la recursion divide sola si hay overflow
    CATEGORY_PARAM = ""  # se quita: combinado con category_ids=183454 devolvia 0 resultados
    LIMIT = 100
...
def snipe_job(item_id, row_num, fase):
    try:
        resp = ebay_auth.peticion_con_retry(f"https://api.ebay.com/buy/browse/v1/item_summary/{quote(item_id, safe='')}")
        if not resp:
        ...
        for item_id, it in items.items():
            ...
            if not closing_time_str:
                detalle_resp = ebay_auth.peticion_con_retry(
                    f"https://api.ebay.com/buy/browse/v1/item_summary/{quote(item_id, safe='')}")
                if not detalle_resp:
                    continue
