...
log.info(f"[Listings] Nuevos agregados: {len(filas_nuevas)}")
...
if row_data.get("status") not in ("Finalizado",):
...
log.info(f"[Auctions] Nuevas: {len(filas_nuevas)} | Snipers activos: {len(scheduled_ids)}")
...
closing_time_str = str(row.get("closing_time", ""))
