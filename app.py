...
class EbaySearchEngine:
    ...
    def buscar_recursivo(self, query, p_min, p_max, sort_order, buying_option, stats, extra_filter=""):
        items_acumulados = {}
        ...
        # NOTA: se elimino la particion recursiva por rango de precio. Para items de
        # coleccion como estas cartas, los precios se concentran en un rango angosto,
        # asi que dividir por precio no reduce el volumen por sub-busqueda (la mitad
        # barata sigue teniendo casi el mismo total) y solo generaba llamadas de
        # "sondeo" extra sin necesidad. En vez de eso, paginamos directo hasta un tope
        # fijo (MAX_ITEMS_POR_QUERY) sobre el rango completo.
    ...
    def ejecutar_motor_general(self, buying_option, sort_order, extra_filter=""):
        stats = {"total_reportado": 0}
        todos_items = {}
        for q in self.QUERIES:
            for r in self.RANGOS:
                resultados = self.buscar_recursivo(q, r[0], r[1], sort_order, buying_option, stats, extra_filter)
...
def sync_auctions():
    ...
    try:
        # Filtro server-side: le pedimos a eBay SOLO las subastas que cierran hoy,
        # en vez de traer TODA la categoria (~3000 items de cualquier fecha) y
        # descartar el 90%+ ya con la paginacion pagada. Esto es lo que
        # disparaba el consumo de cuota descontrolado.
        hoy_cdmx_inicio = datetime.now(TZ_CDMX).replace(hour=0, minute=0, second=0, microsecond=0)
        hoy_cdmx_fin = hoy_cdmx_inicio + timedelta(days=1) - timedelta(seconds=1)
        inicio_utc = hoy_cdmx_inicio.astimezone(timezone.utc).strftime("%Y-%m-%dT%H:%M:%SZ")
        fin_utc = hoy_cdmx_fin.astimezone(timezone.utc).strftime("%Y-%m-%dT%H:%M:%SZ")
        filtro_fecha = f",itemEndDate:[{inicio_utc}..{fin_utc}]"

        items, stats = search_engine.ejecutar_motor_general("AUCTION", "endingSoonest", extra_filter=filtro_fecha)
